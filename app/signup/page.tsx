...
const OTHER_FEATURE_SIGNUP_DISABLED = true; // Toggle this to enable/disable phone input (optional)
...
const SignupPage = () => {
    ...
    const [phoneSignUpDisabled, setPhoneSignUpDisabled] = useState(false);
    ...
    const handleConfirmPasswordInput = handleInput(setConfirmPassword);
            ...
            <div className="w-full max-w-md rounded-2xl border border-white/10 bg-black/50 shadow-[0_20px_80px_rgba(0,0,0,0.6)] backdrop-blur">
                <div className="px-6 pt-7 pb-6">
                    <div className="flex items-center justify-center text-white mb-6">
                        <Image src="/logo_text.png" alt="DevFlow" width={124} height={124} />
                    ...
                    <div className="text-center mb-6">
                        ...
                        <p className="mt-1 text-sm text-white/60">Sign up to build better APIs</p>
                    </div>

                    <div className="space-y-2">
                        <button
                            type="button"
                            disabled={OTHER_FEATURE_SIGNUP_DISABLED}
                            title={OTHER_FEATURE_SIGNUP_DISABLED ? "GitHub signup is temporarily unavailable. Please sign up with email." : ""}
                            className={`w-full h-10 rounded-lg border border-white/10 bg-white/5 text-sm text-white hover:bg-white/10 transition flex items-center justify-center gap-2 ${OTHER_FEATURE_SIGNUP_DISABLED ? "cursor-not-allowed opacity-50" : ""}`}
                        >
                        ...
                        <button
                            type="button"
                            disabled={OTHER_FEATURE_SIGNUP_DISABLED}
                            title={OTHER_FEATURE_SIGNUP_DISABLED ? "Google signup is temporarily unavailable. Please sign up with email." : ""}
                            className={`w-full h-10 rounded-lg border border-white/10 bg-white/5 text-sm text-white hover:bg-white/10 transition flex items-center justify-center gap-2 ${OTHER_FEATURE_SIGNUP_DISABLED ? "cursor-not-allowed opacity-50" : ""}`}
                        >
                            <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-white/10">
                    ...
                    <form onSubmit={handleSignup} className="space-y-3">
                        ...
                        <div>
                            <label className="block text-xs text-white/60 mb-1" htmlFor="confirm-password">Confirm password</label>
                            <div className="relative">
                                <input
                                    id="confirm-password"
                        ...
                        <div>
                            <label className="block text-xs text-white/60 mb-1" htmlFor="phone">
                                Add a phone number <span className="text-white/40">(optional)</span>
                            </label>
                            <input
                                id="phone"
                                type="tel"
                                ...
                                onFocus={() => {
                                    if (OTHER_FEATURE_SIGNUP_DISABLED) setPhoneSignUpDisabled(true);
                                }}
                                onClick={() => {
                                    if (OTHER_FEATURE_SIGNUP_DISABLED) setPhoneSignUpDisabled(true);
                                }}
                                className={`w-full h-10 rounded-lg border px-3 text-sm outline-none transition ${OTHER_FEATURE_SIGNUP_DISABLED
                                ...
                                disabled={OTHER_FEATURE_SIGNUP_DISABLED || loading}
                                autoComplete="tel"
                            />
                            {(OTHER_FEATURE_SIGNUP_DISABLED && phoneSignUpDisabled) && (
                                <div className="mt-2 text-xs text-red-400">
                                    Phone/OTP signup is temporarily unavailable. Please sign up with email.
                                </div>
                            )}
                        </div>
